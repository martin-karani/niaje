...
// Enums
export const documentTypeEnum = pgEnum('document_type', ['lease_agreement', 'tenant_id', 'inspection_report', 'property_photo', 'unit_photo', 'maintenance_invoice', 'notice', 'other']);
export const documentStorageProviderEnum = pgEnum('document_storage_provider', ['local', 's3', 'google_cloud_storage', 'azure_blob']);
...
export const documents = pgTable("documents", {
  ...
  relatedPropertyId: text("related_property_id").references(() => properties.id, { onDelete: 'cascade' }),
  relatedUnitId: text("related_unit_id").references(() => units.id, { onDelete: 'cascade' }),
  relatedLeaseId: text("related_lease_id").references(() => leases.id, { onDelete: 'cascade' }),
  relatedTenantId: text("related_tenant_id").references(() => tenants.id, { onDelete: 'cascade' }),
  relatedInspectionId: text("related_inspection_id").references(() => inspections.id, { onDelete: 'cascade' }),
  ...
  storageProvider: documentStorageProviderEnum("storage_provider").default('local').notNull(),
  ...
  uploadedBy: text("uploaded_by").references(() => user.id, { onDelete: 'set null' }), // User who uploaded
...
export const documentsRelations = relations(documents, ({ one }) => ({
  ...
  unit: one(units, { fields: [documents.relatedUnitId], references: [units.id] }),
  lease: one(leases, {
