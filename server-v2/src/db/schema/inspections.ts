...
// Enums
export const inspectionTypeEnum = pgEnum('inspection_type', ['move_in', 'move_out', 'periodic', 'drive_by', 'safety', 'other']);
export const inspectionStatusEnum = pgEnum('inspection_status', ['scheduled', 'completed', 'canceled', 'pending_report']);
...
export const inspections = pgTable("inspections", {
  ...
  leaseId: text("lease_id").references(() => leases.id, { onDelete: 'set null' }), // Link to lease for move-in/out
...
export const inspectionsRelations = relations(inspections, ({ one, many }) => ({
  organization: one(organization, { fields: [inspections.organizationId], references: [organization.id] }),
  ...
  lease: one(leases, { fields: [inspections.leaseId], references: [leases.id] }),
  inspector: one(user, {
