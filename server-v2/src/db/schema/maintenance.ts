...
// Enums
export const maintenanceStatusEnum = pgEnum('maintenance_status', ['reported', 'scheduled', 'in_progress', 'on_hold', 'completed', 'canceled', 'requires_owner_approval']);
export const maintenancePriorityEnum = pgEnum('maintenance_priority', ['low', 'medium', 'high', 'urgent']);
export const maintenanceCategoryEnum = pgEnum('maintenance_category', ['plumbing', 'electrical', 'hvac', 'appliances', 'structural', 'landscaping', 'pest_control', 'cleaning', 'other']);
...
export const maintenanceRequestsRelations = relations(maintenanceRequests, ({ one, many }) => ({
  organization: one(organization, { fields: [maintenanceRequests.organizationId], references: [organization.id] }),
