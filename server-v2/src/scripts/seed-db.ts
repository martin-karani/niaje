...
async function seedDatabase(resetFirst: boolean = false) {
  ...
  const existingAdmin = await db.query.user.findFirst({
    where: eq(schema.user.email, "admin@propertysystem.com"),
  });
  ...
  for (let i = 0; i < Math.min(agentStaffUsers.length, 20); i++) {
    ...
    const existingMembership = await db.query.member.findFirst({
      where: and(
        ...
        eq(schema.member.userId, staffUser.id)
      ),
    });
  ...
  for (let i = 0; i < Math.min(60, leaseIds.length * tenantIds.length); i++) {
    const leaseId = leaseIds[i % leaseIds.length];
    const tenantId = tenantIds[i % tenantIds.length];

    // Check if assignment already exists
    const existingAssignment = await db.query.leaseTenants.findFirst({
      where: and(
        eq(schema.leaseTenants.leaseId, leaseId),
        eq(schema.leaseTenants.tenantId, tenantId)
      ),
    });
    ...
    if (!existingAssignment) {
      ...
      await db.insert(schema.leaseTenants).values({
        id: leaseTenantId,
        leaseId: leaseId,
        tenantId: tenantId,
  ...
  // Step 12: Create communications
  console.log("Step 12: Creating communications...");
  ...
  // Step 13: Create documents
  console.log("Step 13: Creating documents...");
  ...
  for (let i = 0; i < 40; i++) {
    ...
    if (docType === "property_photo" && propertyIds.length > 0) {
      ...
      if (relatedUnitId) {
        const unit = await db.query.units.findFirst({
          where: eq(schema.units.id, relatedUnitId),
        });
        if (unit) {
      ...
      if (relatedLeaseId) {
        const lease = await db.query.leases.findFirst({
          where: eq(schema.leases.id, relatedLeaseId),
        });
        if (lease) {
  ...
  console.log("Step 14: Creating expenses...");
  ...
  // Step 15: Create utility bills
  console.log("Step 15: Creating utility bills...");
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (lease) {
      const leaseTenant = await db.query.leaseTenants.findFirst({
        where: eq(schema.leaseTenants.leaseId, lease.id),
      });
